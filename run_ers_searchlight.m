function run_ers_searchlight(iteration, varargin)
...
% memory trial types
memtrialtypes = {'RecHits' 'FamHits' 'Misses'};
...
encoding_STM_path  = fullfile(glm_path, 'SingleTrialEncodingModel', subject, 'SPM.mat:spm');
...
% Now we are going to further limit the trials to hits ONLY
...
% diagonal to be weighted 1/number_of_hits for the hits and
% -1/number_of_mises for the mises
switch memtrialtype
    case 'RecHits'
        Weighted_RecHits_Diag = remember_response_Filt / length(find(remember_response_Filt));
        theDiagnol            = Weighted_RecHits_Diag;
    case 'FamHits'
        Weighted_FamHits_Diag = familiar_response_Filt / length(find(familiar_response_Filt));
        theDiagnol            = Weighted_FamHits_Diag;
    case 'Misses'
        Weighted_Misses_Diag  = new_response_Filt / length(find(new_response_Filt));
        theDiagnol            = Weighted_Misses_Diag;
end

template_matrix = diag(theDiagnol);

% Use kyles_cosmo_ers_measure.
...
outfilename = sprintf('sub-%s_memtrialtype-%s_ers-searchight.nii', subject, memtrialtype);
