function run_ers_roi(iteration, varargin)
...
if isempty(varargin)
    ds_enc  = cosmo_fmri_dataset(encoding_STM_path, 'Mask', '/gpfs/group/nad12/default/nad12/FAME8/RSA/models/ERS_results/InferiorFrontalGyrus_from_spmT_0001spmT_0001.img');
    ds_ret  = cosmo_fmri_dataset(retrieval_STM_path, 'Mask', '/gpfs/group/nad12/default/nad12/FAME8/RSA/models/ERS_results/InferiorFrontalGyrus_from_spmT_0001spmT_0001.img');
else
    ds_enc = varargin{1};
    ds_ret = varargin{2};
end
...
raw = cosmo_correlation_measure(ds_stacked, 'output', 'raw');

figure;
imagesc(cosmo_unflatten(raw, 1));
colorbar();
title('Raw ERS Matrix')

nclasses = length(unique(ds_stacked.sa.targets));
